...
execute if entity @s[scores={buildType=11}] run summon villager ~ ~0.5 ~ {Tags:["adv_vis","new_adv_vis"],NoGravity:1b,Silent:1b,Invulnerable:1b,NoAI:1b,Age:-999999,VillagerData:{profession:"minecraft:mason",type:"minecraft:snow"},ArmorItems:[{},{},{},{id:"minecraft:gold_nugget",Count:1b}]}
...
execute if entity @s[scores={buildType=1}] run scoreboard players set @s buildCost 0
...
execute if entity @s[scores={buildType=11}] run scoreboard players set @s buildCost 0

#set health
execute if entity @s[scores={buildType=1}] run scoreboard players set @s buildHealthMax 999999

execute if entity @s[tag=store2,scores={buildCost=0}] run summon armor_stand ~ ~1.7 ~ {Tags:["text","newtext","cost"],CustomNameVisible:1b,Small:1b,Marker:1b,Invisible:1b,CustomName:'{"text":"Cost 0","color":"dark_aqua"}'}
