...
execute unless entity @s[tag=traitor1] run function game:player/draw
execute unless entity @s[tag=traitor2] run function game:player/draw
...
execute as @e[tag=advisor,tag=!store2,scores={buildType=2}] if score @s player_num = @p[tag=turn] player_num as @p[tag=turn] run function game:player/draw
...
function game:player/search_for_next_player
