...
execute if entity @s[scores={player_cards=1..}] if entity @e[tag=!advisor,tag=building,tag=!store,tag=target_seen,tag=built,scores={buildType=1..}] if entity @e[tag=selected,tag=built,scores={buildType=1..}] run function ig:player/click/start_battle
execute if entity @s[scores={player_cards=..0}] run tellraw @s {"text":"No cards to attack with."}

execute unless entity @e[tag=building,tag=target_seen] if entity @e[tag=!advisor,tag=building,tag=selected,tag=!store,tag=action_build] run function ig:player/click/search_targets

execute if entity @e[tag=building,tag=selected,tag=store] run function ig:player/click/start_store
execute if entity @e[tag=building,tag=selected,tag=store2] run function ig:player/click/start_store
